...
if [[ -d /Volumes/Private/Recording/ ]]; then
    n=`ls /nosync/RecordingToSend/*.MP3 | wc -l`
    echo "There are $n mp3 file(s)."
    if [[ "$n" -ge 1 ]]; then
        zmv -v "/nosync/RecordingToSend/*_(*)_(*)A0.MP3" '/Volumes/Private/Recording/Recorder/$1_$2.MP3'
    fi
    n=`ls /nosync/RecordingToSend/*.m4a | wc -l`
